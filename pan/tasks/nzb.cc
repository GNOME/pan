  ...
  Quark const *path = nullptr)
{
  ...
  SQLite::Statement q(pan_db, R"SQL(
    ...
    join article as a on a.id == x.article_id
    where a.message_id == ?
    order by g.name asc
  )SQL");
