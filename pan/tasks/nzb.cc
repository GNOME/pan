...
#include "nzb.h"
#include "pan/data/outbound-article.h"
  ...
  Quark const *path = nullptr)
{
  ...
  // now for the parts...
  out << indent(depth++) << "<segments>\n";
  for (OutboundArticle::part_iterator it(a.pbegin()), end(a.pend()); it!=end; ++it)
  {
    std::string mid = it.mid ();
...
NZB :: upload_list_to_xml_file (std::ostream   & out,
                   const std::vector<OutboundArticle*> & tasks)
{

  foreach_const (std::vector<OutboundArticle*>, tasks, it)
  {
