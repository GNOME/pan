...
struct TaskArchive
{
    ...
    virtual void save_tasks(std::vector<Task *> const &saveme UNUSED)
    {
    }
  ...
  private AdaptableSet<Task *, TaskWeakOrdering>::Listener
{
  public:
    ...
    void stop_tasks(tasks_t const &);
    void restart_tasks(tasks_t const &);
    void remove_tasks(tasks_t const &);
    void move_up(tasks_t const &);
    void move_down(tasks_t const &);
    void move_top(tasks_t const &);
    void move_bottom(tasks_t const &);
    ...
    void add_tasks(tasks_t const &, AddMode = AGE);
    ...
    void get_task_speed_KiBps(Task const *,
                              double &setme_KiBps,
                              int &setme_connections) const;
    void get_connection_counts(int &setme_active,
                               int &setme_idle,
    ...
    void get_full_connection_counts(
  ...
  public:
    ...
    struct task_states_t
    {
      ...
      public:
        ...
        {
          if (_decoding && (task == _decoding))
          {
            return DECODING;
          }
          if (_encoding && (task == _encoding))
          {
            return ENCODING;
          }
          if (_removing.count(task))
          {
            return REMOVING;
          }
          if (_stopped.count(task))
          {
            return STOPPED;
          }
          if (_running.count(task))
          {
            return RUNNING;
          }
          if (_need_decode.count(task))
          {
            return QUEUED_FOR_DECODE;
          }
          if (_need_encode.count(task))
          {
            return QUEUED_FOR_ENCODE;
          }
          if (_queued.count(task))
          {
            return QUEUED;
          }
          return STOPPED;
    ...
    Task const *operator[](size_t i) const
    {
      return _tasks[i];
    }
  ...
  public:
    ...
    struct Listener
    {
        ...
        virtual void on_queue_error(Queue &, StringView const &message) = 0;
    };
  ...
  private: // inherited from NNTP_Pool::Listener
    virtual void on_pool_has_nntp_available(Quark const &server);
    virtual void on_pool_error(Quark const &server, StringView const &message);
  ...
  protected:
    ...
    bool _is_online;
    Task *find_first_task_needing_server(Quark const &server);
    ...
    bool find_best_server(Task::State::unique_servers_t const &servers,
                          Quark &setme);
    bool task_is_active(Task const *) const;
  ...
  public:
    void fire_queue_error(StringView const &message);

  private:
    typedef Loki::AssocVector<Quark, NNTP_Pool *> pools_t;
    pools_t _pools;
    NNTP_Pool &get_pool(Quark const &server);
    void request_connections(quarks_t const &servers);
  ...
  public:
    void set_task_save_delay(int value)
    {
      _save_delay_secs = value;
    }
  ...
  private:
    ...
    void on_set_item_moved(TaskSet &,
                           Task *&,
                           int index,
