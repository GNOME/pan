...
void Queue ::on_pool_has_nntp_available(Quark const &server)
{
...
void Queue ::on_pool_error(Quark const &server UNUSED,
                           StringView const &message)
{
  fire_queue_error(message);
}

NNTP_Pool &Queue ::get_pool(Quark const &servername)
{
  ...
  {
    pool = new NNTP_Pool(
      servername, _server_info, _prefs, _socket_creator, _certstore);
...
****
***/
void Queue ::clean_n_save()
{

  tasks_t const tmp(_tasks.begin(), _tasks.end());
...
void Queue ::fire_if_counts_have_changed()
{
  ...
  bool const count_changed(previous_count != count);
  ...
  bool const counts_changed(active != prev_active || total != prev_total);
  prev_active = active;
  prev_total = total;
  if (counts_changed)
  {
...
void Queue ::get_task_counts(int &active, int &total)
{
...
void Queue ::give_task_a_decoder(Task *task)
{
  bool const was_active(task_is_active(task));
  _decoder_task = task;
  if (! was_active)
  {
...
void Queue ::give_task_a_encoder(Task *task)
{
  bool const was_active(task_is_active(task));
  _encoder_task = task;
  if (! was_active)
...
void Queue ::give_task_a_connection(Task *task, NNTP *nntp)
{
  bool const was_active(task_is_active(task));
...
void Queue ::give_task_an_upload_slot(TaskUpload *task)
{
...
void Queue ::give_task_a_download_slot(TaskArticle *task)
{
...
void Queue ::process_task(Task *task)
{
...
Task *Queue ::find_first_task_needing_decoder()
{
  foreach (TaskSet, _tasks, it)
  {
    Task::State const &state((*it)->get_state());
    if ((state._work == Task::NEED_DECODER) && (! _stopped.count(*it))
        && (! _removing.count(*it)))
    {
      return *it;
    }
...
Task *Queue ::find_first_task_needing_encoder()
{
  foreach (TaskSet, _tasks, it)
  {
    Task::State const &state((*it)->get_state());
    if ((state._work == Task::NEED_ENCODER) && (! _stopped.count(*it))
        && (! _removing.count(*it)))
    {
      return *it;
    }
...
Task *Queue ::find_first_task_needing_server(Quark const &server)
{
  foreach (TaskSet, _tasks, it)
  {
    Task::State const &state((*it)->get_state());
    if (state._health != ERR_COMMAND && state._health != ERR_LOCAL
        && state._health != ERR_NOSPACE && (state._work == Task::NEED_NNTP)
        && (state._servers.count(server)) && (! _stopped.count(*it))
        && (! _removing.count(*it)))
    {
      return *it;
    }
...
bool Queue ::find_best_server(Task::State::unique_servers_t const &servers,
                              Quark &setme)
{
  int max_score(0);
  Quark best_server;
  ...
  foreach_const (Task::State::unique_servers_t, servers, it)
  {
    ...
    NNTP_Pool const &pool(get_pool(server));
    ...
    if (_is_online)
    {
      ...
      int const empty_slots(max - (idle + active));
...
void Queue ::on_set_items_added(TaskSet &container UNUSED,
                                TaskSet::items_t &tasks,
                                int pos)
{
  _needs_saving = true;
...
void Queue ::on_set_item_removed(TaskSet &container UNUSED,
                                 Task *&task,
                                 int pos)
{
  _needs_saving = true;
...
void Queue ::on_set_item_moved(TaskSet &container UNUSED,
                               Task *&task,
                               int new_pos,
                               int old_pos)
{
...
void Queue ::fire_queue_error(StringView const &message)
{
...
void Queue ::fire_tasks_added(int pos, int count)
{
  for (lit it(_listeners.begin()), end(_listeners.end()); it != end;)
  {
...
void Queue ::fire_task_removed(Task *&task, int pos)
{
  for (lit it(_listeners.begin()), end(_listeners.end()); it != end;)
  {
...
void Queue ::fire_task_moved(Task *&task, int new_pos, int old_pos)
{
  for (lit it(_listeners.begin()), end(_listeners.end()); it != end;)
  {
...
void Queue ::fire_connection_count_changed(int count)
{
  for (lit it(_listeners.begin()), end(_listeners.end()); it != end;)
  {
...
void Queue ::fire_size_changed(int active, int total)
{
  for (lit it(_listeners.begin()), end(_listeners.end()); it != end;)
  {
...
void Queue ::fire_online_changed(bool online)
{
  for (lit it(_listeners.begin()), end(_listeners.end()); it != end;)
  {
...
void Queue ::fire_task_active_changed(Task *task, bool active)
{
  for (lit it(_listeners.begin()), end(_listeners.end()); it != end;)
  {
...
void Queue ::set_online(bool online)
{
  _is_online = online;
...
void Queue ::stop_tasks(tasks_t const &tasks)
{
...
void Queue ::restart_tasks(tasks_t const &tasks)
{
  foreach_const (tasks_t, tasks, it)
  {
    Task *task(*it);
...
void Queue ::add_task(Task *task, AddMode mode)
{
  tasks_t tasks;
...
void Queue ::add_tasks(tasks_t const &tasks, AddMode mode)
{
...
bool Queue ::contains(Quark const &mid) const
{
  return _mids.count(mid);
}

void Queue ::remove_latest_task()
{
  if (! _tasks.empty())
  {
...
bool Queue ::task_is_active(Task const *task) const
{
...
void Queue ::remove_tasks(tasks_t const &tasks)
{
  foreach_const (tasks_t, tasks, it)
  {
    remove_task(*it);
...
void Queue ::remove_task(Task *task)
{
  int const index(_tasks.index_of(task));
...
void Queue ::get_all_task_states(task_states_t &setme)
{
  setme.tasks.clear();
...
void Queue ::check_in(NNTP *nntp, Health nntp_health)
{
  ...
  if ((nntp_health != ERR_NETWORK) && _is_online && state._health != ERR_COMMAND
      && state._health != ERR_LOCAL && state._health != ERR_NOSPACE
      && (state._work == Task::NEED_NNTP) && ! _removing.count(task)
...
void Queue ::check_in(Decoder *decoder UNUSED, Task *task)
{
...
void Queue ::check_in(Encoder *encoder UNUSED, Task *task)
{
  // take care of our decoder counting...
  _encoder_task = nullptr;
...
void Queue ::move_up(tasks_t const &tasks)
{
  foreach_const (tasks_t, tasks, it)
  {
    Task *task(*it);
    int const old_pos(_tasks.index_of(task));
    _tasks.move_up(old_pos);
  }
}

void Queue ::move_down(tasks_t const &tasks)
{
...
void Queue ::move_top(tasks_t const &tasks)
{
  foreach_const_r(tasks_t, tasks, it)
  {
    Task *task(*it);
...
void Queue ::move_bottom(tasks_t const &tasks)
{
  foreach_const (tasks_t, tasks, it)
  {
    Task *task(*it);
...
void Queue ::get_connection_counts(int &setme_active,
                                   int &setme_idle,
...
void Queue ::get_full_connection_counts(
  std::vector<ServerConnectionCounts> &setme) const
{
  ...
  foreach_const (pools_t, _pools, it)
  {
    ...
    it->second->get_counts(
      counts.active, counts.idle, counts.connecting, unused);
...
double Queue ::get_speed_KiBps() const
{
  double KiBps(0.0);
  foreach_const (nntp_to_task_t, _nntp_to_task, it)
  {
...
void Queue ::get_task_speed_KiBps(Task const *task,
                                  double &setme_KiBps,
...
void Queue ::get_stats(unsigned long &queued_count,
                       ...
                       int &seconds_remain)
{
  ...
  foreach_const (tasks_t, tasks.tasks, it)
  {
    ...
    Queue::TaskState const state(tasks.get_state(task));
  ...
  unsigned long const task_count(running_count + queued_count);
  KiBps = get_speed_KiBps();
  if (task_count)
  {
    double const KiB((double)KiB_remain / 1024);
    unsigned long tmp(KiBps > 0.01 ? ((unsigned long)(KiB / KiBps)) : 0);
    seconds_remain = tmp % 60ul;
    tmp /= 60ul;
    minutes_remain = tmp % 60ul;
    tmp /= 60ul;
    hours_remain = tmp;
  }
}

void Queue ::on_dl_limit_reached()
{
...
void Queue ::on_reset_xfer_bytes()
{
  set_online(true);
}
