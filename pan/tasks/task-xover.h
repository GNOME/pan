...
class TaskXOver : public Task, private NNTP::Listener
{
  public: // life cycle
    ...
    TaskXOver(Data &data,
              Quark const &group,
              Mode mode = ALL,
              unsigned long sample_size = 1000);
    virtual ~TaskXOver();
  ...
  private: // NNTP::Listener
    virtual void on_nntp_line(NNTP *, StringView const &) override;
    virtual void on_nntp_done(NNTP *, Health, StringView const &) override;
    ...
    void on_nntp_line_process(NNTP *, StringView const &);
  ...
  private: // implementation - minitasks
    struct MiniTask
    {
        ...
        MiniTask(Type type,
                 Article_Number low = static_cast<Article_Number>(0ul),
                 Article_Number high = static_cast<Article_Number>(0ul)) :
          _type(type),
          _low(low),
          _high(high)
        {
