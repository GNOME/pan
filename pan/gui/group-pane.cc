  ...
  public:
    void get_value(int column, GValue *setme) override
    {
      switch (column)
      {
        ...
        //
        case COL_UNREAD:
          set_value_ulong(setme, static_cast<uint64_t>(unread));
          break;
      }
    }
};
...
{
  return ! name.empty() && name != *sub_title_quark
         && name != *other_title_quark && name != *virtual_title_quark;
}
...
std::string get_short_name(StringView const &in)
{
  ...
  while (myline.pop_token(tok, '.'))
  {
    out.insert(out.end(),
               tok.begin(),
               ((tok == long_token_one || tok == long_token_two) ?
                  tok.end() :
                  tok.begin() + 1));
    out += '.';
  }
...
Quark GroupPane ::get_first_selection() const
{
  Quark group;
...
quarks_v GroupPane ::get_full_selection() const
{
  quarks_v groups;
  ...
  for (GList *l(list); l; l = l->next)
  {
    ...
    {
      groups.push_back(
        dynamic_cast<MyRow *>(_tree_store->get_row(&iter))->groupname);
    }
  }
...
void GroupPane ::do_popup_menu(GtkWidget *,
...
{
  static_cast<GroupPane *>(pane_g)->_action_manager.activate_action(
    "read-selected-group");
  return false;
}
} // namespace

void GroupPane ::on_row_activated(GtkTreeView *,
                                  GtkTreePath *,
                                  GtkTreeViewColumn *,
                                  gpointer pane_g)
{
  // user may have activated by selecting the row... wait for the
  // GtkTreeSelection to catch up
...
void row_collapsed_or_expanded_cb(GtkTreeView *,
                                  GtkTreeIter *,
                                  GtkTreePath *,
                                  gpointer)
{
...
gboolean GroupPane ::on_button_pressed(GtkWidget *treeview,
                                       GdkEventButton *event,
                                       gpointer userdata)
{
...
gboolean GroupPane ::on_popup_menu(GtkWidget *treeview, gpointer userdata)
{
...
void delete_rows(gpointer data, GObject *)
{
  ...
  std::string group_str(R"SQL(
    select distinct name, (
      select count() from `article` as ia
		    join `article_xref` as x on ia.id == x.article_id
        where x.group_id = g.id and ia.is_read = False
			) as count
      from `group` as g
      join `server_group` as sg on sg.group_id == g.id
      join `server` as s on s.id == sg.server_id
...
void GroupPane ::on_group_subscribe(Quark const &groupname, bool sub)
{
  ...
  if (gtk_tree_model_iter_nth_child(
        model, &section_iter, nullptr, (sub ? 1 : 2)))
  {
    if (gtk_tree_model_iter_children(model, &group_iter, &section_iter))
    {
      do
      {
  ...
  // move the row
  _tree_store->reparent(
    _tree_store->get_row(&section_iter), find_row(groupname), pos);
...
void GroupPane ::refresh_dirty_groups()
{
...
gboolean GroupPane ::dirty_groups_idle(gpointer user_data)
{
...
void entry_icon_released(GtkEntry *,
...
void clear_button_clicked_cb(GtkEntry *e,
                             GtkEntryIconPosition icon_pos,
                             GdkEventButton *,
...
void search_menu_toggled_cb(GtkCheckMenuItem *menu_item, gpointer entry_g)
{
  if (gtk_check_menu_item_get_active(menu_item))
  {
    ...
    GroupPane *h =
      (GroupPane *)g_object_get_data(G_OBJECT(entry_g), "group-pane");
    bump_activate_soon_tag(h);
  }
}
} // namespace

void GroupPane ::on_grouplist_rebuilt()
{
...
void render_group_name(GtkTreeViewColumn *,
                       ...
                       gpointer gp)
{
  ...
  std::string group_name(do_shorten ? get_short_name(StringView(name)) :
                                      name.to_string());
  ...
  std::string bg_col(
    pane->get_prefs().get_color_str_wo_fallback("group-pane-color-bg"));
  if (bg_col.empty())
  {
    bg_col = def_bg;
  }
  ...
  g_object_set(renderer,
               ...
               group_name.c_str(),
               "weight",
               (! is_g || static_cast<uint64_t>(unread) != 0 ?
                  PANGO_WEIGHT_BOLD :
                  PANGO_WEIGHT_NORMAL),
               "foreground",
               fg_col.empty() ? nullptr : fg_col.c_str(),
               "background",
               bg_col.empty() ? nullptr : bg_col.c_str(),
               nullptr);
}
} // namespace

void GroupPane ::set_name_collapse(bool b)
{
  shorten = b;
 ...
 */
GtkTreePath *GroupPane ::find_next_subscribed_group(bool unread_only)
{
  GtkTreeView *view(GTK_TREE_VIEW(_tree_view));
  ...
  for (;;)
  {
    ...
    if (! is_virtual
        && (static_cast<uint64_t>(row->unread) != 0 || ! unread_only))
    {
...
void GroupPane ::read_group(StringView const &groupname)
{
  ...
  GtkTreePath *path =
    gtk_tree_model_get_path(gtk_tree_view_get_model(view), &iter);
  read_group(path);
  gtk_tree_path_free(path);
}

void GroupPane ::read_group(GtkTreePath *path)
{
  g_assert(path);
...
void GroupPane ::read_next_group_impl(bool unread_only)
{
...
void GroupPane ::read_next_unread_group()
{
  read_next_group_impl(true);
}

void GroupPane ::read_next_group()
{
...
namespace {
gboolean select_func(GtkTreeSelection *,
                     GtkTreeModel *model,
                     GtkTreePath *path,
                     gboolean,
                     gpointer)
...
GtkWidget *GroupPane ::create_filter_entry()
{
  ...
  g_signal_connect(
    entry, "icon-release", G_CALLBACK(clear_button_clicked_cb), this);
  entry_changed_tag = g_signal_connect(
    entry, "changed", G_CALLBACK(search_entry_changed_by_user), this);
  ...
  g_signal_connect(
    entry, "icon-release", G_CALLBACK(entry_icon_released), menu);
...
void GroupPane ::on_selection_changed(GtkTreeSelection *,
                                      gpointer pane_gpointer)
{
  ...
  {
    self->_action_manager.sensitize_action(actions_that_require_a_group[i],
                                           have_group);
  }
  ...
  {
    self->_action_manager.hide_action(actions_in_nonvirtual_group[i],
                                      is_virtual_group(group));
  }
}

GroupPane ::GroupPane(ActionManager &action_manager,
                      Data &data,
                      Prefs &prefs,
  ...
  _dirty_groups_idle_tag(0)
{
  ...
  expand_iterators(
    iters, GTK_TREE_MODEL(_tree_store), GTK_TREE_VIEW(_tree_view));

  GtkTreeSelection *selection(
    gtk_tree_view_get_selection(GTK_TREE_VIEW(_tree_view)));
  gtk_tree_selection_set_mode(selection, GTK_SELECTION_MULTIPLE);
  gtk_tree_selection_set_select_function(
    selection, select_func, nullptr, nullptr);
  g_signal_connect(
    selection, "changed", G_CALLBACK(on_selection_changed), this);
  on_selection_changed(selection, this);

  g_signal_connect(_tree_view,
                   "row_collapsed",
                   G_CALLBACK(row_collapsed_or_expanded_cb),
                   nullptr);
  g_signal_connect(_tree_view,
                   "row_expanded",
                   G_CALLBACK(row_collapsed_or_expanded_cb),
                   nullptr);
  g_signal_connect(
    _tree_view, "button-press-event", G_CALLBACK(on_button_pressed), this);
  g_signal_connect(
    _tree_view, "button-release-event", G_CALLBACK(on_button_pressed), this);
  g_signal_connect(_tree_view, "popup-menu", G_CALLBACK(on_popup_menu), this);
  g_signal_connect(
    _tree_view, "row-activated", G_CALLBACK(on_row_activated), this);
