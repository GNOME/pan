                                        ...
                                        bool const do_thread)
{
  ...
  int count(0);
  std::vector<Data::ArticleTree::ArticleChild> children;
  ...
  count += children.size();
  for (Data::ArticleTree::ArticleChild it : children)
  {
    rows.push_back(create_row(it.a));
  }
  ...
  for (size_t i = 0, n = children.size(); i < n; ++i)
  {
    if (children[i].has_child)
      {
        count += add_children_to_model(store, group, rows[i], children[i].a.message_id, atree, do_thread);
      }
  }
  return count;
}
...
void HeaderPane ::update_tree()
{
  ...
  SQLite::Statement is_old_selection_shown(pan_db, R"SQL(
    select show from article_view
    join article on article_view.article_id == article.id
    where message_id == ? and show == True
  )SQL");
  ...
  LOG4CXX_TRACE(logger, "nb of hidden articles: " << hidden.size());
  ...
  if (do_thread)
  {
    auto insert_reparented_row =
      [this, do_thread, &reparented](Quark msg_id, Quark prt_id)
    {
