...
OutboundParts ::OutboundParts(OutboundParts const &that) :
  n_parts_total(0),
  part_mid_buf_len(0),
  parts(0),
  part_mid_buf(nullptr)
...
OutboundParts &OutboundParts ::operator=(OutboundParts const &that)
{
  if (this != &that)
  {
    clear();
...
void OutboundParts ::unpack_message_id(std::string &setme,
                                   Part const *part,
...
bool OutboundParts ::get_part_info(number_t part_number,
...
void OutboundParts ::set_parts(OutboundPartBatch const &p)
{
  clear();
  ...
  Part part;
  OutboundPartBatch::parts_t::const_iterator in = p.parts.begin();
...
bool OutboundParts ::add_part(number_t num,
                      StringView const &mid,
                      bytes_t bytes,
...
OutboundPartBatch::OutboundPartBatch() :
  n_parts_found(0),
  n_parts_total(0),
  packed_mids_len(0)
{
...
OutboundPartBatch::OutboundPartBatch(Quark const &mid, number_t parts_total) :
  reference_mid(mid),
  n_parts_found(0),
...
void OutboundPartBatch ::init(Quark const &mid, number_t parts_total)
{
  reference_mid = mid;
  packed_mids_len = 0;
  n_parts_total = parts_total;
...
void OutboundPartBatch ::add_part(number_t number, StringView const &mid, bytes_t bytes)
{
...
OutboundPartBatch ::Part &OutboundPartBatch ::Part ::operator=(OutboundPartBatch ::Part const &that)
{
  if (this != &that)
  {
    number = that.number;
...
OutboundPartBatch ::Part ::Part(OutboundPartBatch::Part const &that) :
  number(that.number),
  bytes(that.bytes),
  len_used(that.len_used),
  packed_mid(new char[len_used])
...
OutboundPartBatch ::Part ::Part(number_t n, bytes_t b, size_t l) :
  number(n),
  bytes(b),
  len_used(l),
  packed_mid(new char[len_used])
