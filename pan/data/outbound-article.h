...
#ifndef __Outbound_Article_h__
#define __Outbound_Article_h__

#include <ctime>
#include <pan/data/outbound-parts.h>
...
// article may rely on another class, like OutboundArticle and
// OutboundParts.
class OutboundArticle
{
  public:
    bool add_part(OutboundParts::number_t num,
                  StringView const &mid,
                  OutboundParts::bytes_t bytes)
    {
      return parts.add_part(num, mid, bytes, message_id);
    }

    void set_part_count(OutboundParts::number_t num)
    {
      parts.set_part_count(num);
    }

    OutboundParts::number_t get_total_part_count() const
    {
      return parts.get_total_part_count();
    }

    OutboundParts::number_t get_found_part_count() const
    {
      return parts.get_found_part_count();
    }

    bool get_part_info(OutboundParts::number_t num,
                       std::string &mid,
                       OutboundParts::bytes_t &bytes) const
    {
      return parts.get_part_info(num, mid, bytes, message_id);
    }

    typedef OutboundParts::const_iterator part_iterator;
