...
InMemoryParts ::InMemoryParts(InMemoryParts const &that) :
  n_parts_total(0),
  part_mid_buf_len(0),
  parts(0),
  part_mid_buf(nullptr)
...
InMemoryParts &InMemoryParts ::operator=(InMemoryParts const &that)
{
  if (this != &that)
  {
    clear();
...
void InMemoryParts ::unpack_message_id(std::string &setme,
                                   Part const *part,
...
bool InMemoryParts ::get_part_info(number_t part_number,
...
void InMemoryParts ::set_parts(InMemoryPartBatch const &p)
{
  clear();
  ...
  Part part;
  InMemoryPartBatch::parts_t::const_iterator in = p.parts.begin();
...
bool InMemoryParts ::add_part(number_t num,
                      StringView const &mid,
                      bytes_t bytes,
...
InMemoryPartBatch::InMemoryPartBatch() :
  n_parts_found(0),
  n_parts_total(0),
  packed_mids_len(0)
{
...
InMemoryPartBatch::InMemoryPartBatch(Quark const &mid, number_t parts_total) :
  reference_mid(mid),
  n_parts_found(0),
...
void InMemoryPartBatch ::init(Quark const &mid, number_t parts_total)
{
  reference_mid = mid;
  packed_mids_len = 0;
  n_parts_total = parts_total;
...
void InMemoryPartBatch ::add_part(number_t number, StringView const &mid, bytes_t bytes)
{
...
InMemoryPartBatch ::Part &InMemoryPartBatch ::Part ::operator=(InMemoryPartBatch ::Part const &that)
{
  if (this != &that)
  {
    number = that.number;
...
InMemoryPartBatch ::Part ::Part(InMemoryPartBatch::Part const &that) :
  number(that.number),
  bytes(that.bytes),
  len_used(that.len_used),
  packed_mid(new char[len_used])
...
InMemoryPartBatch ::Part ::Part(number_t n, bytes_t b, size_t l) :
  number(n),
  bytes(b),
  len_used(l),
  packed_mid(new char[len_used])
