...
void DataImpl ::load_headers_from_db(Quark const &group) {
  ...
  SQLite::Statement read_article_q(pan_db,R"SQL(
    select flag,message_id, subject, author.name || " <" || author.address || ">",
                             ...
                             unsigned long &article_count)
{
  ...
  else
  {
    ...
    foreach_const (std::vector<Article *>, articles, ait) {
      ...
      set_author_q.reset();
      StringView author_name, author_address, author(a->author.to_view());
      author.pop_token(author_name,'<');
      author_name.trim();
      author.pop_token(author_address, '>');
      ...
      set_article_q.bind(4, author_address);
