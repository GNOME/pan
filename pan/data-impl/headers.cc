...
void DataImpl ::update_part_states(Quark const &group)
{
  ...
  SQLite::Statement q(pan_db,R"SQL(
    ...
    from (
       ...
       from article as a
       join article_xref as x on x.article_id == a.id
       join `group` as g on x.group_id == g.id
       where g.name == ?
...
void DataImpl::store_references(Quark message_id, std::string references)
{
  ...
  // real child articles that points to the ghost to be replace
  ...
  if (changed != 0) {
    // found a real article, so ghost article must be deleted
    SQLite::Statement delete_ghost(pan_db, R"SQL(
      delete from ghost where ghost_msg_id == ?
    )SQL");
    delete_ghost.bind(1, message_id);
    delete_ghost.exec();
    LOG4CXX_TRACE(tree_logger, "references msg " << message_id << ": delete ghost article");
  }
...
void DataImpl ::load_headers_from_db(Quark const &group) {
    ...
    with goners ( article_id ) as (
      ...
      where g.name == ?
        ...
        (
          ...
          select count()
          from article_xref as x
          join server       as s on x.server_id == s.id
          where x.article_id == a.id
            and (
              -- server has no expiration set
              s.expiry_days == 0 or
              -- article is younger than expiration date
              julianday() - julianday(a.time_posted, 'unixepoch') < s.expiry_days
            )
  ...
  if (expire_count) {
    LOG4CXX_DEBUG(logger, "Expired " << expire_count << " articles from group " << group.c_str()
                  << "in " << timer.get_seconds_elapsed() << "s.");
                          ...
                          bool read)
{
  ...
  SQLite::Statement read_group_q(pan_db, R"SQL(
    ...
    join article as a on x.article_id == a.id
    where message_id = ?
  )SQL");
...
void DataImpl ::rescore_group_articles(Quark const &group)
{
  ...
  SQLite::Statement q(pan_db, R"SQL(
    ...
    join article_xref as x on x.article_id == a.id
    join `group` as g on g.id == x.group_id
    where g.group = ?
  )SQL");
...
void DataImpl ::on_articles_added(Quark const &group, quarks_t const &mids)
{
  ...
  if (! mids.empty())
  {
    ...
    foreach (std::set<MyTree *>, _trees, it)
    {
      LOG4CXX_DEBUG(logger,"This tree has a group " << (*it)->_group);
      if ((*it)->_group == group)
      {
        LOG4CXX_DEBUG(logger,"trying to add the articles to tree " << *it);
