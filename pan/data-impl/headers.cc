...
void DataImpl ::migrate_headers(DataIO const &data_io, Quark const &group)
{
  ...
  LOG4CXX_INFO(_db_logger,  "Migrating articles of groups " << group.c_str()
  ...
  if (in && ! in->fail())
  {
    ...
    if (version == 1 || version == 2 || version == 3)
    {
      ...
      for (;;)
      {
        ...
        Quark author = s.len == 1 ? author_lookup[(int)*s.str] : Quark(s);
        set_author_q.bind(1,author);
        set_author_q.exec();
        // set author id in article table
        set_article_q.bind(bind_idx++,author.c_str());
        ...
        set_article_q.bind(bind_idx++,total_part_count); // expected_parts in table
  ...
  LOG4CXX_INFO(_db_logger, "Migrated " << item_count << " articles and parts of groups " << group.c_str()
...
void DataImpl ::load_headers_from_db(Quark const &group) {
  ...
  LOG4CXX_INFO(_db_logger, "Loading headers for group " << group.c_str());
  ...
  SQLite::Statement read_article_q(pan_db,R"SQL(
    select flag,message_id, subject, author.author, time_posted, binary, expected_parts, line_count, `references`
      from article
      join author on author_id = author.id
      join article_xref as xrf on xrf.article_id = article.id
      where xrf.group_id = ?;
)SQL");
  ...
  SQLite::Statement read_xref_q(pan_db,R"SQL(
    select s.pan_id, grp.name, xrf.number, s.expiry_days
      ...
      join article as a on xrf.article_id = a.id
      join `group` as grp on xrf.group_id = grp.id
  ...
  LOG4CXX_INFO(_db_logger, "Loaded " << article_count << " articles in " << timer.get_seconds_elapsed() << "s.");
                             ...
                             unsigned long &article_count)
{
  ...
  LOG4CXX_INFO(_db_logger, "Saving new articles of groups " << group.c_str()
               << " in DB...");
  ...
  else
  {
    ...
    foreach_const (std::vector<Article *>, articles, ait) {
      ...
      foreach_const (Xref, a->xref, xit)
      {
        ...
        set_xref_q.bind(2, xit->group.c_str());
        set_xref_q.bind(3, xit->server.c_str());
                          ...
                          bool read)
{
       ...
       ++it)
  {
    ...
    foreach_const (Xref, article->xref, xit)
    {
      ...
      set_read_q.bind(2, article->message_id);
      assert(set_read_q.exec() == 1);
      LOG4CXX_TRACE(_db_logger,  "Setting read status of article " << article->message_id << " to " << read);
...
bool DataImpl ::is_read(Article const *a) const
{
  ...
  LOG4CXX_FATAL(_db_logger, "Article not found in DB "
...
void DataImpl ::group_clear_articles(Quark const &group)
{
  ...
  // remove 'em from DB too. delete the xref. A DB trigger cleans up orphaned article
  SQLite::Statement delete_q(pan_db, R"SQL(
    delete from `article_xref` where group_id == (select id from `group` where name == ?);
  )SQL" );
  delete_q.bind(1,group.c_str());
  int count = delete_q.exec();

  LOG4CXX_DEBUG(_db_logger,  "Deleted all " << count << " articles of group "
                << group.c_str() << " in DB.");
