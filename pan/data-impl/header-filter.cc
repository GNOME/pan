...
log4cxx::LoggerPtr logger = pan::getLogger("header-filter");
...
) const
{
  ...
  {
    to_test = "subject";
    join = "join subject on subject.article_id == article.id";
  }
  if (header_name == from)
  {
    to_test = "address";
      ...
      else
      {
        res.push_back(SqlCond("("));
        {
          foreach_const (FilterInfo::aggregatesp_t, criteria._aggregates, it)
          {
            ...
            if ((*it)->_needs_body)
            {
              res.push_back(SqlCond("article.cached = ?", (*it)->_needs_body));
            }
            else
            {
