...
log4cxx::LoggerPtr db_logger = pan::getLogger("database");
...
quarks_t DataImpl ::get_server_ids_from_db () const {
  ...
  LOG4CXX_DEBUG(db_logger,"get_servers: got " << servers.size() << " servers from DB");
...
namespace
{
  ...
  void DataImpl ::migrate_server_properties_into_db(DataIO const &source)
  {
    ...
    foreach_const (key_to_keyvals_t, spc.data, it)
    {
      ...
      Server s;
      pan_debug("loading server id " << it->first << " from file");
      ...
      LOG4CXX_INFO(_db_logger, "Migrating server " << s.host );
...
{
  LOG4CXX_DEBUG(db_logger, "saving data of server" << s->host << " in DB");
  ...
  if (nb > 0) {
    LOG4CXX_INFO(_db_logger, "created server " << s->host << " in DB");
  }
...
{
  LOG4CXX_DEBUG(db_logger, "deleting server" << host << " from DB");
  ...
  LOG4CXX_INFO(_db_logger, "Removed server " << host << "from DB.");
}
