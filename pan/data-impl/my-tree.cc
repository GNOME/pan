...
int DataImpl ::MyTree ::initial_call_on_shown_articles(
  ...
  std::string q = R"SQL(
    with recursive hierarchy (step, a_id, m_id, p_id, sort_data) as (
      ...
      join hierarchy as h
	    where status is not "h" and av.parent_id is not null and av.parent_id = h.a_id
    ...
    from hierarchy
    order by step, prt_msg_id, sort_data )SQL" + (sort_ascending ? "asc" : "desc");
  ...
  std::function<void(Quark msg_id, Quark parent_id)> cb) const
{
  ...
  std::string q = R"SQL(
    with recursive hierarchy as (
      ...
      from article_view as av
	    join article as a on a.id == av.article_id
      where av.status is "e" and av.parent_id is null
      ...
      from article_view as av
	    join article as a on a.id == av.article_id
      join hierarchy as h
	    where av.status is "e" and av.parent_id is not null
        and av.parent_id = h.article_id
	    limit 10000000 -- todo remove ?
    )
...
void DataImpl ::MyTree ::update_article_view() const {
  ...
  LOG4CXX_TRACE(logger, "commit done in "
                            << timer.get_seconds_elapsed() - tmp_time << "s).");
