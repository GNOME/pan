                                          ...
                                          std::vector<Article> &setme) const
{
  ...
  if (mid.empty())
  {
    LOG4CXX_DEBUG(logger, "Initial load on article_view table");
    reset_article_view();
    ...
    {
      return "insert into article_view (article_id, parent_id, shown)\n"
             ...
             + join + " where g.name == ? and " + where
             + " order by article.id";
    };

    auto q = _header_filter.get_sql_query(_data, c, criteria);
    q.bind(1, group);
    int count = q.exec();
  ...
  _save_path(save_path)
{
  ...
  load_db_schema_file(pan_db, "temp_article.sql");
}
...
void DataImpl ::MyTree ::apply_rules(const_nodes_v &candidates)
{
  ...
  foreach (const_nodes_v, candidates, it)
  {
    ...
    if (! _data._article_rules.apply_rules(
          _data, _rules, _group, *(*it)->_article))
    {
