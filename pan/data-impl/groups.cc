...
void DataImpl ::migrate_newsrc(Quark const &server_name, LineReader *in) {
  ...
  LOG4CXX_DEBUG(_db_logger, "saving groups of server " << server->host << " in DB");
  ...
  LOG4CXX_INFO(_db_logger, "Migrated " << count << " groups of server " << server->host
                << " in DB in " << seconds << "s.");
}
...
void DataImpl ::load_groups_from_db() {
  ...
  foreach_const (servers_t, _servers, sit) {
    ...
    LOG4CXX_INFO(_db_logger, "Loaded " << i << " groups from server with pan_id " << server.c_str() << " from DB in "
  ...
  LOG4CXX_DEBUG(_db_logger, "loaded " << s_count << " subscribed groups from DB");
  ...
  LOG4CXX_DEBUG(_db_logger,"loaded " << u_count << " unsubscribed groups from DB");

  LOG4CXX_INFO(_db_logger, "Loaded " << s_count + u_count << " group subscription info from DB in "
               << timer.get_seconds_elapsed() << "s." );
...
void DataImpl::save_new_groups_in_db(Quark const &server_pan_id, NewGroup const *newgroups, int count) {
  TimeElapsed timer;
  LOG4CXX_DEBUG(_db_logger, "Saving " << count << " new groups in DB");
  ...
  LOG4CXX_DEBUG(_db_logger, "Saving new groups in DB. Stored old groups: " << timer.get_seconds_elapsed() << " s");
  ...
  LOG4CXX_DEBUG(_db_logger, "Saving new groups in DB. Prepared statements: " << timer.get_seconds_elapsed() << " s");
  ...
  for (NewGroup const *it = newgroups, *end = newgroups + count; it != end; ++it) {
    ...
    if (old_groups.count(name) == 0) {
      LOG4CXX_TRACE(_db_logger, "Adding new group " << name << " permission " << it->permission );
      SQLite::Transaction store_group_transaction(pan_db);
      ...
      LOG4CXX_TRACE(_db_logger, "Updating group " << name << " permission " << it->permission );
    ...
    if (perm == 'n' || perm == 'm' || perm == 'y') {
      ...
      LOG4CXX_DEBUG(_db_logger, "Skipping permission '" << perm << "' of group " << name << " server pan_id " << server_pan_id.c_str() );
    }
  ...
  LOG4CXX_DEBUG(_db_logger, "Saving new groups in DB. Checked or saved groups: " << timer.get_seconds_elapsed() << " s");

  int deleted(0);
  SQLite::Statement delete_group_q(pan_db, R"SQL(
    delete from `server_group` where group_id = (select id from `group` where name = ? and server_id = ?) ;
)SQL");
  ...
  for (auto const& grp : old_groups) {
    LOG4CXX_DEBUG(_db_logger, "Deleting obsolete group: " << grp.first);
    delete_group_q.reset();
    delete_group_q.bind(1, grp.first);
    delete_group_q.bind(2, server_id);
    int count = delete_group_q.exec();
    assert(count == 1);
    deleted++;
  }

  LOG4CXX_DEBUG(_db_logger, "Saving new groups in DB. Deleted obsolete groups: " << timer.get_seconds_elapsed() << " s");

  pan_db.exec("pragma synchronous = normal");

  LOG4CXX_INFO(_db_logger, "Added " << added << " and deleted " << deleted <<
...
void DataImpl::save_group_in_db(Quark const &server_name) {
  ...
  LOG4CXX_INFO(_db_logger, "Saving groups of server " << server->host << " in DB...") ;
  ...
  LOG4CXX_INFO(_db_logger, "Saved " << count << " groups "
               << "in " << seconds << "s in DB.");
  pan_db.exec("pragma synchronous = normal");
}
...
void DataImpl ::migrate_group_descriptions(DataIO const &data_io) {
  ...
  LOG4CXX_INFO(_db_logger,  "Migrating group descriptions from " << filename << "..." );
  ...
  // remove obsolete file
  std::remove(filename.data());
  LOG4CXX_INFO(_db_logger, "Migration of group descriptions done.");
}
...
void DataImpl ::migrate_group_xovers(DataIO const &data_io)
{
  ...
  if (in && !in->fail())
  {
    LOG4CXX_INFO(_db_logger, "Migrating newsgroup.xov file to DB");
    StringView line;
    ...
    LOG4CXX_INFO(_db_logger, "Migrated " << count << " records from newsgroup.xov into DB" );
...
void DataImpl ::load_group_xovers_from_db() {
  TimeElapsed timer;
  LOG4CXX_DEBUG(_db_logger,"Loading xovers data from DB");
  ...
  LOG4CXX_INFO(_db_logger, "Loaded " << count << " group xover info from DB in "
               << timer.get_seconds_elapsed() << "s.");
}
...
DataImpl :: save_group_xovers ()
{
  ...
  foreach_const (xgroups_t, xgroups, it)
  {
    ...
    count += exec_count;
    if ( exec_count != 1) {
      LOG4CXX_WARN(_db_logger, "Unkwown group " << groupname << " when saving xover data");
    };
    ...
    foreach_const (ReadGroup::servers_t, g._servers, i) {
      if (static_cast<uint64_t>(i->second._xover_high) != 0) {
        ...
        if (exec_count != 1) {
          LOG4CXX_WARN(_db_logger, "Unkwown group " << groupname << " or server pan_id " << i->first.c_str() << " when saving xover data in DB");
        };
      }
    }
...
void DataImpl ::mark_group_read(Quark const &groupname)
{
  SQLite::Statement set_read_q(pan_db, R"SQL(
    update `article` as a set is_read = True
      where a.id in (
         select article_id from `article_xref` as xrf, `group` as g
         where g.name = ? and xrf.group_id == g.id
    ) and is_read == False
)SQL");
  ...
  LOG4CXX_TRACE(_db_logger, "Set " << count <<
...
char DataImpl ::get_group_permission(Quark const &group) const
{
  ...
  assert(perm != nullptr);
  LOG4CXX_TRACE(_db_logger, "got permission " << *perm << " for group " << group.c_str());
