...
void ProfilesImpl ::load_posting_profiles()
{
  SQLite::Statement read(pan_db, R"SQL(
    select name, s.host, p.username, address, face, xface, attribution, fqdn
    from profile as p
...
ProfilesImpl :: save_posting_profiles () const
{
  SQLite::Statement save(pan_db, R"SQL(
    insert into profile
    (name, server_id, username, address, face, xface, attribution, fqdn)
    values ($n, $s_id, $un, $adr, $f, $xf, $atr, $fqdn)
    on conflict (name)
    do update set server_id = $s_id, username = $un , address = $adr,
              face= $f, xface = $xf, attribution = $atr, fqdn = $fqdn
    where name = $name
  )SQL");
  ...
  foreach_const (profiles_t, profiles, it)
  {
    ...
    save.bind(i++, it->second.username);
