...
void ProfilesImpl ::load_posting_profiles()
{
  SQLite::Statement read(pan_db, R"SQL(
    select p.name, s.host, a.name, a.address, face, xface, attribution, fqdn
    from profile as p
  ...
  while (read.executeStep())
  {
    ...
    p.username = read.getColumn(i++).getText();
    p.address = read.getColumn(i++).getText();
...
ProfilesImpl :: save_posting_profiles () const
{
  SQLite::Statement save_author(pan_db, R"SQL(
    ...
    values ($n, $adr)
    on conflict (address)
    do update set name = $n
    where address = $adr
  )SQL");

  SQLite::Statement save(pan_db, R"SQL(
    insert into profile
    (name, server_id, author_id, face, xface, attribution, fqdn)
    values ($n, $s_id, (select id from author where address == $adr), $f, $xf, $atr, $fqdn)
    on conflict (name)
    do update set author_id = (select id from author where address == $adr),
  ...
  {
    save_author.reset();
    save_author.bind(1, it->second.username);
    save_author.bind(2, it->second.address);
    save_author.exec();
