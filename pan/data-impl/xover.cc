...
//  std::cerr<<"xover add : "<<subject<<" "<<author<<" "<<message_id<<" lines "<<line_count<<" bytes "<<byte_count<<std::endl;
  ...
  ***/
  ...
  if (part_count > 1)
  {
    ...
    // find all article with similar subject that may be the right article for this part
    ...
    for (cit it(range.first), end(range.second); it!=end && art_mid.empty(); ++it) {
      ...
      // match for this part
      StringView author_name,author_address, auth(author);
      auth.pop_token(author_name, '<');
      author_name.trim();
      auth.pop_token(author_address, '>');
      search_article_q.reset();
      search_article_q.bind(1, it->second);
      search_article_q.bind(2, author_name);
      search_article_q.bind(3, author_address);
  ...
  if (art_mid.empty())
  {
    ...
    if (count == 0) {
      ...
      set_author_q.bind(1, author_name);
      set_author_q.bind(2, author_address);
      set_author_q.exec();
      ...
      SQLite::Statement create_article_q(pan_db, R"SQL(
        ...
        values ((select id from author where address = ?),?,?,?,?,?)
      )SQL");
      create_article_q.bind(1, author_address);
