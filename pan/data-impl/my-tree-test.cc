...
class DataImplTest : public CppUnit::TestFixture
{
  ...
  public:
    ...
    void assert_unchanged(std::string label, std::string msg_id)
    {
      assert_prop(label, "unchanged", msg_id);
    }
    ...
    void test_get_unread_children_beginning_thread()
    {
      ...
      assert_not_in_view("step 2", "g1m1a");
      assert_not_in_view("step 2", "g1m1b");
    ...
    void test_get_unread_children_middle_thread()
    {
      ...
      assert_hidden("step 2", "g1m1c2");
      assert_unchanged("step 2", "g1m1d1");
      ...
      assert_unchanged("step 3", "g1m1d2");
    }
    ...
    void test_article_sort()
    {
      ...
      pan_db.exec(R"SQL(
        insert into article (message_id,author_id, time_posted, part_state, line_count, bytes)
          values
            ("g1m1", (select id from author where author like "Me%"), 1234, "I", 10, 56),
            ("g1o1", (select id from author where author like "Other%"), 1235, "C", 11, 65);
        insert into subject (article_id, subject) values
            ((select id from article where message_id == "g1m1"), "Bla"),
            ((select id from article where message_id == "g1o1"), "Meh");
      )SQL");
    ...
    void test_function_on_exposed_articles()
    {
      ...
      // we can view the 3 read articles. They are in separate threads
      // so they have no children
      assert_result("step 1", Quark(), "g1", {"g1m1c1", "g1m2", "g1m2a"});
      ...
      assert_result("step 2", Quark(), "g1", {"g1m1a", "g1m2b"});
